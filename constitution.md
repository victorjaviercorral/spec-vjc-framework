...
**F.26-ter Criterio antes de tokens, prueba visual antes de aprobar. [Prototipo+]**
`/design-system` produce dos capas, siempre en este orden: `brand.md` (criterio en lenguaje humano — personalidad, presupuesto de acento, reglas de composición, prohibiciones) codificado después en `tokens.css`. Un `tokens.css` sin `brand.md` es una paleta sin dirección; un `brand.md` sin `tokens.css` es una promesa que cada componente interpreta distinto. Dentro de `tokens.css`, dos prohibiciones concretas contra el aspecto genérico: ninguna curva de easing usa el valor por defecto del navegador (`ease`, `ease-in-out`, `linear`) sin nombre propio ni propósito declarado, y ninguna sombra es una capa única — se compone de anillo de contacto, sombra ajustada y ambiente amplio a opacidad baja. El sistema se aprueba mirando `showcase.html` renderizado en el navegador, nunca solo leyendo la especificación: un sistema que solo existe en prosa no es verificable, y es indistinguible de uno que no funciona hasta que alguien lo mira. Procedencia declarada en `docs/fundamentos.md` §8: aporte externo, sin ejecución propia que lo valide todavía.
