...
12. **Identidad visual propia por proyecto.** Cada proyecto define su `design-identity.md` en /spec-init. Prohibido el look generico por defecto (gradientes violeta, glassmorphism de plantilla, tipografia por defecto). Las skills de diseno (ui-ux-pro-max, taste-skill u otras) se usan como motor, pero la identidad del proyecto las gobierna, no al reves. Para microinteracciones y motion, combinar siempre `emil-design-eng` (pulido de componentes, performance de transform/opacity, curvas de easing propias, reduced-motion correcto) y `apple-design` (fisicidad, manejo directo, momentum/rubber-banding, interrumpibilidad): son complementarias, no redundantes — evidencia: piloto LegoVirtualMuseum (2026-07-27), la propuesta mas innovadora de 4 nacio de usarlas juntas en la misma iteracion, no por separado.
