...
- **Asunción que contrabandea un dato.** Un `AS-nn` que afirme algo sobre el mundo —«los usuarios prefieren X», «la competencia cobra Y», «este segmento pagaría Z»— es hallazgo **alto**, no una asunción válida (constitution A.4-bis, regla operativa). Una asunción describe lo que el autor decide; si describe lo que hacen terceros, es un dato sin fuente reformulado para que pase el control.
- **Requisito no implementable.** Un requisito que no se pueda falsar con un criterio de verificación no es un requisito. Dos formas concretas, ambas hallazgo **medio** porque inflan la densidad sin especificar nada: la aserción de test disfrazada («el sistema deberá tratarlo como defecto bloqueante») y el rechazo de una capacidad que no existe (eso es una exclusión de alcance).
